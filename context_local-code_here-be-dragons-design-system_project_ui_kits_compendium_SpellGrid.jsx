  ...
  return (
    <div style={s.container}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={s.tabs}>
          {schools.map(sc => (
            <button key={sc} style={s.tab(activeSchool === sc)} onClick={() => setActiveSchool(sc)}>{sc}</button>
          ))}
        </div>
        <span style={s.count}>{filtered.length} spell{filtered.length !== 1 ? 's' : ''}</span>
      </div>

      <div style={s.grid}>
        {filtered.length === 0 && (
          <div style={s.empty}>"No such enchantment exists in the known tomes."</div>
        )}
        {filtered.map(spell => (
          <SpellCard
            key={spell.id}
